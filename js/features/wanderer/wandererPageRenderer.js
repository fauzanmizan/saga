...
export const WandererPageRenderer = {
    pageTemplates: {},
    ...
    setupWandererNavEvents() {
        ...
        navLinks.forEach(link => {
            ...
            WandererPageRenderer._navClickHandler = async (e) => {
                ...
                if (currentUser && currentUser.archetype) {
                    let targetNpc = null;
                    const npcsInCurrentRegion = Object.values(dbInstance.npc_souls || {}).filter(npc => npc.currentRegion === currentUser.currentRegion);
                ...
                WandererPageRenderer.renderWandererPage(pageId);
        ...
        WandererPageRenderer.renderWandererPage(initialPage);
    ...
    renderWandererPage(pageId) {
        ...
        allPageContainers.forEach(container => {
            if (container.id.endsWith('-page')) { // Ensure it's a page container
                container.style.display = 'none';
            }
        });

        // Show the requested page container
        const targetPageContainer = document.getElementById(`${pageId}-page`);
    ...
    _getDOMElementsForPage(pageId) {
        ...
        if (pageId === 'inventory') {
            ...
            worldMapSvgContainer = document.getElementById('world-map-svg-container');
        ...
        // No need to fetch elements for quest_log, character, skill_tree, sanctuary here
        // as their content is usually rendered via specific functions or is static enough.
        // Their elements are directly accessed within their rendering functions.
    },
    ...
    renderAllWandererComponents(pageId) {
        ...
        if (pageId === 'character') {
            ...
            WandererPageRenderer.renderWandererScales();
            WandererPageRenderer.renderWandererAltar();
            WandererPageRenderer.renderWandererRadarChart();
            ...
            WandererPageRenderer.renderSanctuaryVisuals();
            WandererPageRenderer.renderSanctuaryAttributeCrystal();
            WandererPageRenderer.renderSanctuaryConscienceScales();
            WandererPageRenderer.renderSanctuaryChronicleWall();
    ...
    _applyCosmicTheme: () => {
        ...
        if (cosmicCycleNameDisplay && cosmicCycleIconDisplay && dbInstance.world.cosmicCycle) {
            const currentCycleDef = COSMIC_CYCLES[dbInstance.world.cosmicCycle.currentCycleId];
    ...
    renderWorldMap() {
        const container = document.getElementById('world-map-page'); // Assuming a container for the world map page
        if (!container) return;

        // Ensure the base HTML for the world map and detail panel is always present when page is 'world_map'
        container.innerHTML = `
            <div id="world-map-svg-container" class="relative w-full h-[600px] bg-slate-900 rounded-lg overflow-hidden border border-border-color mb-8">
        ...
        // Draw regions (as simple rectangles for now, can be complex paths later)
        const regionWidth = 300; // Example dimensions
        const regionHeight = 200;
        let currentRegionX = 0;
        let currentRegionY = 0;

        for (const regionId in regions) {
            const region = regions[regionId];
            const regionRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
            regionRect.setAttribute("x", currentRegionX);
            regionRect.setAttribute("y", currentRegionY);
            regionRect.setAttribute("width", regionWidth);
            regionRect.setAttribute("height", regionHeight);
            regionRect.setAttribute("fill", `url(#${region.terrainType}-pattern)`); // Use patterns for terrain
            regionRect.setAttribute("stroke", "white");
            regionRect.setAttribute("stroke-width", "2");
            regionRect.classList.add('region-svg-shape');
            regionRect.dataset.regionId = regionId; // Store ID for click listener
            mapSvg.appendChild(regionRect);

            // Add text label for region
            const text = document.createElementNS("http://www.w3.org/2000/svg", "text");
            text.setAttribute("x", currentRegionX + regionWidth / 2);
            text.setAttribute("y", currentRegionY + regionHeight / 2);
            ...
            // Simple layout: arrange in a grid/line
            currentRegionX += regionWidth + 20; // Move right for next region
            if (currentRegionX > 900) { // Wrap to next row
                currentRegionX = 0;
                currentRegionY += regionHeight + 20;
            }

            regionRect.addEventListener('click', (e) => {
                WandererPageRenderer._showRegionDetailPanel(regionId, e.clientX, e.clientY);
            });
        }
        
        // Render POI Markers on the map (relative to map SVG container's viewBox)
        const worldLandmarks = WORLD_LANDMARKS || {};
        for (const poiId in worldLandmarks) {
            const poi = worldLandmarks[poiId];
            const region = dbInstance.world.regions[poi.regionId]; // Get region data

            if (region) { // Only render if region exists
                const poiMarker = document.createElementNS("http://www.w3.org/2000/svg", "circle"); // Use circle for simple POI
                const regionMapX = (mapSvg.getBoundingClientRect().width / 1000) * currentRegionX + poi.coords.x * regionWidth; // Simplified positioning, adjust later
                const regionMapY = (mapSvg.getBoundingClientRect().height / 1000) * currentRegionY + poi.coords.y * regionHeight;

                // For better positioning, map coords need to be relative to the *map's SVG viewBox*.
                // Assuming viewBox is 1000x1000, and poi.coords are 0-1 relative to region.
                // This will need careful calculation if regions are not a simple grid.
                // For now, place randomly within region's general area in the SVG's coordinate system.
                // Let's use simple scaling for demo, assuming 1000x1000 viewBox.
                const regionSvgOffset = { // This needs to be calculated based on the actual regionRect positions
                    'TheCentralNexus': { x: 500, y: 500 }, // Center
                    'TheWhisperingReaches': { x: 300, y: 200 },
                    'TheLuminousPlains': { x: 700, y: 800 },
                    'TheShatteredPeaks': { x: 750, y: 250 },
                    'TheCrimsonDesert': { x: 200, y: 600 },
                    'TheAzureForest': { x: 400, y: 800 },
                    'TheSunkenCity': { x: 100, y: 100 },
                    'TheFloatingIslands': { x: 900, y: 400 },
                    'TheAshfallWastes': { x: 400, y: 700 },
                    'TheSilentCanyon': { x: 500, y: 300 },
                    'TheWhisperingWoods': { x: 700, y: 500 },
                    'TheEternalGlacier': { x: 150, y: 850 },
                    'TheVerdantJungle': { x: 850, y: 150 }
                };
                const baseSvgX = regionSvgOffset[poi.regionId]?.x || 500;
                const baseSvgY = regionSvgOffset[poi.regionId]?.y || 500;

                poiMarker.setAttribute("cx", baseSvgX + (poi.coords.x - 0.5) * 150); // Offset from region center
                poiMarker.setAttribute("cy", baseSvgY + (poi.coords.y - 0.5) * 150);
                poiMarker.setAttribute("r", "10"); // Radius
                ...
                if (poi.currentStatus === 'discovered' || poi.currentStatus === 'active') {
                    fill = 'green';
                    iconHtml = `<i data-feather="${poi.icon || 'map-pin'}" class="w-4 h-4 text-white"></i>`;
                } else if (poi.currentStatus === 'rumored') {
                    fill = 'yellow';
                    iconHtml = `<i data-feather="${poi.icon || 'help-circle'}" class="w-4 h-4 text-white"></i>`;
                } else if (poi.currentStatus === 'corrupted') {
                    fill = 'red';
                    iconHtml = `<i data-feather="${poi.icon || 'skull'}" class="w-4 h-4 text-white"></i>`;
                } else if (poi.currentStatus === 'purified') {
                    fill = 'blue';
                    iconHtml = `<i data-feather="${poi.icon || 'sparkles'}" class="w-4 h-4 text-white"></i>`;
                } else if (poi.currentStatus === 'ruined' || poi.currentStatus === 'unknown') {
                    fill = 'darkgray';
                    iconHtml = `<i data-feather="${poi.icon || 'x'}" class="w-4 h-4 text-white"></i>`;
                }
                
                poiMarker.setAttribute("fill", fill);
                poiMarker.setAttribute("stroke", stroke);
                poiMarker.classList.add('poi-marker');
                poiMarker.dataset.poiId = poiId;
                poiMarker.style.cursor = 'pointer';

                mapSvg.appendChild(poiMarker);

                // Add an SVG foreignObject for feather icon within the SVG
                const foreignObject = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
                foreignObject.setAttribute("x", parseFloat(poiMarker.getAttribute("cx")) - 10); // Offset for icon centering
                foreignObject.setAttribute("y", parseFloat(poiMarker.getAttribute("cy")) - 10);
                foreignObject.setAttribute("width", 20);
                foreignObject.setAttribute("height", 20);
                foreignObject.innerHTML = iconHtml; // Inject feather icon HTML
                mapSvg.appendChild(foreignObject);


                poiMarker.addEventListener('click', (e) => {
                    WandererPageRenderer._showPoiDetailPanel(poi, e.clientX, e.clientY);
                });
            }
        }


        // Re-render Feather icons for the map
        ...
        // Attach event listeners for zoom buttons and detail panels after rendering
        document.getElementById('map-zoom-in')?.addEventListener('click', () => WandererPageRenderer._handleMapZoom(0.1));
        document.getElementById('map-zoom-out')?.addEventListener('click', () => WandererPageRenderer._handleMapZoom(-0.1));
        document.getElementById('close-poi-detail-panel')?.addEventListener('click', () => WandererPageRenderer._hidePoiDetailPanel());

        // Initial render of NPC list for the current region on map page
        WandererPageRenderer.renderNpcList(getCurrentUser().currentRegion);
    },
    ...
    _showRegionDetailPanel(regionId, mouseX, mouseY) {
        ...
        // Calculate position relative to the map SVG container
        let panelLeft = mouseX - mapRect.left + 20;
        let panelTop = mouseY - mapRect.top + 20;

        // Adjust if panel goes off screen
        if (panelLeft + regionDetailPanel.offsetWidth > mapRect.width) {
            panelLeft = mapRect.width - regionDetailPanel.offsetWidth - 20;
        }
        if (panelTop + regionDetailPanel.offsetHeight > mapRect.height) {
            panelTop = mapRect.height - regionDetailPanel.offsetHeight - 20;
        }
        ...
        document.getElementById('detail-fortify-btn').onclick = async () => {
            ...
            if (user.essenceOfWill >= cost) {
                ...
                UIManagerRef.showNotification(`Nexus ${region.name} diperkuat!`, 'plus-circle', 'success'); // Using 'success' type for color
                WandererPageRenderer._hideRegionDetailPanel();
                WandererPageRenderer.renderWorldMap();
            } else {
    ...
    _showPoiDetailPanel: (poi, mouseX, mouseY) => {
        ...
        poiDetailStatus.textContent = poi.currentStatus.replace(/_/g, ' '); // Use currentStatus

        poiDetailDescription.textContent = REGIONS_DATA[poi.regionId]?.description || 'Tidak ada deskripsi.'; // Fetch region description
        ...
        if (poi.journalEntryId) {
            const journalEntry = JOURNAL_ENTRY_TEMPLATES[poi.journalEntryId]; // Use template data
            ...
            if (journalEntry && isDiscoveredInJournal) {
                poiDetailJournalEntry.innerHTML = `Entri Jurnal: <span class="text-indigo-300 italic">"${journalEntry.title}"</span>`;
                poiDetailJournalEntry.style.display = 'block';
                 // Optionally add a button to view journal entry
                // const viewJournalBtn = document.createElement('button');
                // viewJournalBtn.textContent = 'View Journal';
                // viewJournalBtn.onclick = () => { UIManagerRef.closeModal(); WandererPageRenderer.renderWandererPage('journal'); };
                // poiDetailPanel.appendChild(viewJournalBtn);
            } else if (journalEntry && poi.currentStatus === 'rumored') {
                poiDetailJournalEntry.innerHTML = `Entri Jurnal: <span class="text-slate-500 italic">"Rumor tentang ${journalEntry.title}"</span>`;
                poiDetailJournalEntry.style.display = 'block';
            }
             else {
        ...
        if (panelLeft + poiDetailPanel.offsetWidth > mapRect.width) {
            panelLeft = mapRect.width - poiDetailPanel.offsetWidth - 20;
        }
        if (panelTop + poiDetailPanel.offsetHeight > mapRect.height) {
            panelTop = mapRect.height - poiDetailPanel.offsetHeight - 20;
        }
    ...
    _handleMapZoom(delta) {
        ...
        document.querySelectorAll('.region-svg-shape, .poi-marker, #world-map-svg text, #world-map-svg foreignObject').forEach(element => {
            if (element.tagName === 'rect' || element.tagName === 'circle') {
                 // For SVG shapes, we apply transform on the SVG group or directly on element.
                 // For now, let's just re-render everything on zoom.
                 // This is inefficient but simpler for quick implementation.
                 // For proper zoom, use SVG transform property directly.
            } else {
                const currentTransform = element.style.transform;
                const newTransform = currentTransform.replace(/scale\([^)]+\)/, '').trim();
                element.style.transform = `${newTransform} scale(${currentZoomLevel})`;
            }
        });
        // Re-render the map to apply zoom visually to everything including positions
        WandererPageRenderer.renderWorldMap(); // Re-render for full zoom effect
    },

    renderSkillTree() {
        const container = document.getElementById('skill-tree-page'); // Assuming this ID for skill tree page
        if (!container) return;
        ...
        if (userArchetype && SKILL_TREE_DATA[userArchetype]) {
            ...
            Object.keys(archetypeSkills).forEach(skillKey => {
                ...
                if (skill.level) { // Check for attribute level requirement
                ...
                } else if (!isUnlocked) { // Skills without level requirement but not yet unlocked
                    canUnlock = true;
                }
        ...
        container.innerHTML = html; // Set the full HTML
        if (typeof lucide !== 'undefined' && lucide.createIcons) {
            lucide.createIcons();
        }
    },
    ...
    renderCurrentPage(pageId) {
        ...
        const allPageContainers = document.querySelectorAll('#wanderer-page-container > div'); // Assuming direct children are pages
        allPageContainers.forEach(container => {
            if (container.id.endsWith('-page') || container.id === 'quest-log-page') { // Include quest-log-page
                container.style.display = 'none';
            }
        });
    ...
    renderNpcList: (regionId) => {
        ...
        npcListContainer.innerHTML = '<h3 class="text-xl font-serif font-bold text-white mb-4">NPC di Wilayah Ini:</h3>';
        const npcsInRegion = Object.values(dbInstance.npc_souls || {}).filter(npc => npc.currentRegion === regionId);
    ...
    renderWorldMap() {
        ...
        container.innerHTML = `
            <div id="world-map-svg-container" class="relative w-full h-[600px] bg-slate-900 rounded-lg overflow-hidden border border-border-color mb-8 flex-grow">
                <svg id="world-map-svg" viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid meet" class="w-full h-full">
                    <!-- Definitions for patterns (e.g., terrain textures) -->
                    <defs>
                        <pattern id="plains-pattern" patternUnits="userSpaceOnUse" width="10" height="10">
                            <rect x="0" y="0" width="10" height="10" fill="#4CAF50"/>
                            <circle cx="5" cy="5" r="2" fill="#8BC34A"/>
