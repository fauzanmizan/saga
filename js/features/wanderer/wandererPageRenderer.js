...
import { SKILL_TREE_DATA, GLOBAL_ATTRIBUTES, BIRTH_QUESTIONS, INTERROGATION_DATA, TRADABLE_ITEMS_DATA, COSMIC_CYCLES, NPC_HEALTH_STATES, NPC_LIFESTAGES, NPC_ROLES, NPC_PERSONALITY_TRAITS, WORLD_LANDMARKS, REGIONS_DATA, JOURNAL_ENTRY_TEMPLATES } from '../../data/core.js'; // Ensure all necessary data modules are imported.
...
import { ChallengeGame } from '../../miniGames/challengeGame.js'; // Added import for ChallengeGame
import { InspireGame } from '../../miniGames/inspireGame.js'; // Added import for InspireGame
...
export const WandererFeatures = {
    ...
    setDependencies(db, saveDB, destinyClockInterval) {
        dbInstanceRef = db; // Corrected: Using dbInstanceRef
        ...
        // Note: WandererPageRenderer.setDependencies is called later in initWandererPage with all its dependencies
        // This is a common point of confusion.
        // It should pass `dbInstanceRef` not `db` if `dbInstanceRef` is the intended global reference.
        // However, the `db` parameter here is the actual instance passed from App.js.
        // So this is correct as long as WandererPageRenderer.setDependencies also uses its own parameters.
        // The issue was in the *other* usage of `dbInstance` without `Ref`.
    },
    ...
    initWandererPage() {
        ...
        WandererPageRenderer.renderCurrentPage('character'); // Changed from renderWandererPage
    ...
    // Note: The global variables were moved to the top for better organization.
    ...
    setupWandererNavEvents() {
        ...
        navLinks.forEach(link => {
            ...
            WandererPageRenderer._navClickHandler = async (e) => {
                ...
                if (currentUser && currentUser.archetype) {
                    let targetNpc = null;
                    // Changed dbInstance to dbInstanceRef
                    ...
                    // Assuming WandererGameLogic is correctly imported/available or passed as dependency
                    // Replaced with direct calls to imported mini-game modules
                    switch (pageId) {
                ...
                WandererPageRenderer.renderCurrentPage(pageId); // Changed from renderWandererPage
        ...
        WandererPageRenderer.renderCurrentPage(initialPage); // Changed from renderWandererPage
        WandererPageRenderer.renderWandererNav(initialPage);
    },

    // Renamed from renderWandererPage to renderCurrentPage to avoid confusion
    // as renderWandererPage was the old function signature in the template.
    // The previous implementation of renderWandererPage (in definePageTemplates) was already replaced.
    // This function will now be the main entry point for rendering any page.
    renderCurrentPage(pageId) {
        // Hide all page containers first
        const allPageContainers = document.querySelectorAll('#wanderer-page-container > div'); // Assuming direct children are pages
        allPageContainers.forEach(container => {
            if (container.id.endsWith('-page') || container.id === 'quest-log-page') { // Ensure it's a page container
                container.style.display = 'none';
            }
        });

        // Show the requested page container
        const targetPageContainer = document.getElementById(`${pageId}-page`) || document.getElementById('quest-log-page'); // Fallback for quest_log
        if (targetPageContainer) {
            targetPageContainer.style.display = 'block';

            // Special handling for quest_log since its HTML is part of a single block
            if (pageId === 'quest_log') {
                // The inner content of quest_log-page is handled by setupQuestLogTabs
                // and its render functions. No need to set targetPageContainer.innerHTML here.
            } else {
                targetPageContainer.innerHTML = WandererPageRenderer.pageTemplates[pageId];
            }

            // Get DOM elements specific to this page *after* it's visible
            ...
            if (pageId === 'inventory') {
                ...
                // Close region detail panel on outside click
                ...
                // Close POI detail panel on outside click
    ...
    _getDOMElementsForPage(pageId) {
        // Re-fetch common elements
        Object.assign(this, {
            mainContentArea: document.getElementById('wanderer-page-container'), // Main container
            navLinksContainer: document.getElementById('wanderer-nav')
        });

        // Fetch elements specific to the current page
        if (pageId === 'inventory') {
            ...
            worldMapSvgContainer = document.getElementById('world-map-svg'); // Target the SVG itself
            regionDetailPanel = document.getElementById('region-detail-panel');
            poiDetailPanel = document.getElementById('poi-detail-panel');
            if (poiDetailPanel) { // Check if element exists before getting sub-elements
                ...
                document.getElementById('poi-detail-description'); // Fetch this too
            }
        }
    },
    ...
    renderAllWandererComponents(pageId) {
        ...
        if (pageId === 'character') {
            ...
            // WandererPageRenderer.renderWandererScales(); // This function is not defined in the provided code
            // WandererPageRenderer.renderWandererAltar(); // This function is not defined in the provided code
            // WandererPageRenderer.renderWandererRadarChart(); // This function is not defined in the provided code
            // WandererPageRenderer.renderWandererMandate(); // This function is not defined in the provided code
            // WandererPageRenderer.renderAttributeXpBars(); // This function is not defined in the provided code
            WandererPageRenderer.renderNpcList(currentUser.currentRegion); // Render NPC list for current region
            checkQuestCompletion();
        } else if (pageId === 'quest_log') {
            WandererPageRenderer.setupQuestLogTabs();
            checkQuestCompletion();
        } else if (pageId === 'journal') { // NEW: Render Journal Page
            WandererPageRenderer.renderJournalPage();
        } else if (pageId === 'sanctuary') {
            // WandererPageRenderer.renderSanctuaryVisuals(); // Not defined
            // WandererPageRenderer.renderSanctuaryAttributeCrystal(); // Not defined
            // WandererPageRenderer.renderSanctuaryConscienceScales(); // Not defined
            // WandererPageRenderer.renderSanctuaryChronicleWall(); // Not defined
            WorldManagerRef.updateResonanceDisplay();
        } else if (pageId === 'world_map') {
            WandererPageRenderer.renderWorldMap();
            WandererPageRenderer.renderNpcList(currentUser.currentRegion); // Render NPC list for current region on map page
        } else if (pageId === 'skill_tree') {
            WandererPageRenderer.renderSkillTree();
        } else if (pageId === 'inventory') {
            WandererPageRenderer.renderInventoryPage();
        }
        // Lucide icons are now created at the end of renderWandererPage
    },
    ...
    renderNpcList: (regionId) => {
        ...
        npcsInRegion.forEach(npc => {
            ...
            // NEW: Display NPC Role and Personality Traits
    ...
    renderChronicle() {
        ...
        const currentUserChronicle = getCurrentUser().chronicle; // Using getCurrentUser()
    ...
    renderNotificationLog() {
        ...
        const notificationLog = dbInstanceRef.world.notificationLog || []; // Corrected: Using dbInstanceRef
    ...
    renderWorldMap() {
        ...
        poiDetailType = document.getElementById('poi-detail-type');
        poiDetailJournalEntry = document.getElementById('poi-detail-journal-entry');
        const poiDetailDescription = document.getElementById('poi-detail-description'); // Added for POI description

        if (!worldMapSvgContainer) return;

        // Render regions
        const regions = dbInstanceRef.world.regions || {}; // Corrected: Using dbInstanceRef
        const mapSvg = worldMapSvgContainer;
        mapSvg.innerHTML = ''; // Clear previous regions/markers

        // Define a layout for regions in the 1000x1000 viewBox
        const regionLayout = {
        ...
        // Add pattern definitions to SVG (must be inside <defs>)
        ...
        for (const regionId in regions) {
            ...
            // Draw region background (using pattern for terrain)
            ...
            // Add Nexus State overlay (a translucent rectangle on top)
            ...
            nexusOverlay.classList.add('nexus-overlay', `nexus-status-${region.status.toLowerCase()}`); // CSS will handle colors/opacity
            ...
            text.style.pointerEvents = 'none'; // Make text not interfere with click
            mapSvg.appendChild(text);

            regionRect.addEventListener('click', (e) => {
                const rect = e.target.getBoundingClientRect(); // Get position of the clicked region rect
                WandererPageRenderer._showRegionDetailPanel(regionId, rect.right, rect.top); // Show panel near the rect
            });
        }

        // Render POI Markers
        const worldLandmarks = WORLD_LANDMARKS || {}; // Using the imported global constant
        for (const poiId in worldLandmarks) {
            const poi = worldLandmarks[poiId];
            const region = regions[poi.regionId];

            if (region && regionLayout[poi.regionId]) { // Only render if region and its layout exist
                const regionLayoutData = regionLayout[poi.regionId];

                // Calculate POI position within its region's layout boundaries
                ...
                const markerGroup = document.createElementNS("http://www.w3.org/2000/svg", "g"); // Group for marker and icon
                markerGroup.setAttribute("transform", `translate(${poiSvgX}, ${poiSvgY}) scale(${1 / currentZoomLevel})`); // Scale icon inversely to zoom
                ...
                circle.setAttribute("r", "10"); // Base radius for the circle background

                let fill = 'gray'; // Default
                if (poi.currentStatus === 'active' || poi.currentStatus === 'discovered') fill = '#4CAF50'; // Green
                else if (poi.currentStatus === 'rumored') fill = '#FFC107'; // Amber
                else if (poi.currentStatus === 'corrupted') fill = '#F44336'; // Red
                else if (poi.currentStatus === 'purified') fill = '#2196F3'; // Blue
                else if (poi.currentStatus === 'ruined' || poi.currentStatus === 'unknown') fill = '#607D8B'; // Grey
                ...
                // Add an SVG foreignObject for Feather icon
                const foreignObject = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
                foreignObject.setAttribute("x", -8); // Center the icon
                ...
                markerGroup.addEventListener('click', (e) => {
                    const groupRect = markerGroup.getBoundingClientRect(); // Get position of the group
    ...
    _showRegionDetailPanel(regionId, mouseX, mouseY) {
        ...
        const region = dbInstanceRef.world.regions[regionId]; // Corrected: Using dbInstanceRef
        ...
        const mapContainer = document.getElementById('world-map-svg-container'); // Use the container of SVG
        const mapRect = mapContainer.getBoundingClientRect();

        let panelLeft = mouseX + 20; // Position relative to viewport
        let panelTop = mouseY + 20;

        // Adjust if panel goes off screen
        if (panelLeft + regionDetailPanel.offsetWidth > window.innerWidth) { // Check against window width
            panelLeft = mouseX - regionDetailPanel.offsetWidth - 20;
        }
        if (panelTop + regionDetailPanel.offsetHeight > window.innerHeight) { // Check against window height
    ...
    _showPoiDetailPanel: (poi, mouseX, mouseY) => {
        ...
        // Corrected: Using dbInstanceRef for region description
        ...
        if (poi.journalEntryId) {
            ...
            const currentUserJournal = getCurrentUser().journal || []; // Using getCurrentUser()
    ...
    _handleMapZoom(delta) {
        ...
        // Re-render the map to apply zoom visually to everything including positions
        WandererPageRenderer.renderWorldMap();
    },
    ...
    renderSkillTree() {
        ...
        GLOBAL_ATTRIBUTES.forEach(attrName => { // Iterate through GLOBAL_ATTRIBUTES
            const user = getCurrentUser();
            const attribute = user.attributes.find(a => a.name === attrName);
            const skillData = SKILL_TREE_DATA[attrName]; // Get skills for this attribute
            ...
            if (skillData) {
                // Filter out non-numeric keys (like archetype skills)
        ...
        if (userArchetype && SKILL_TREE_DATA[userArchetype]) {
            ...
            Object.keys(archetypeSkills).forEach(skillKey => {
                ...
                if (skill.level) {
                    ...
                    const requiredLevel = skill.attributeRequirement ? skill.attributeRequirement.level : skill.level; // Use skill.level if no explicit attributeRequirement
                    ...
                    canUnlock = true; // Skills without level requirement but not yet unlocked
                }
            ...
            sanctuary: `
                <div id="sanctuary-page" class="h-full flex flex-col">
                    <div id="soul-sanctuary-container" class="glass-card p-8 rounded-2xl shadow-lg border border-border-color flex-grow flex flex-col">
                        ...
                        <div id="sanctuary-visuals-wrapper" class="w-full h-full relative flex-grow">
    ...
    // NOTE: This function was already present in the user's provided code,
    // but its name was duplicated with the actual renderWandererPage used in setupWandererNavEvents.
    // I've kept the name `renderCurrentPage` for clarity to distinguish from the original
    // `renderWandererPage` (which was a template placeholder) and
    // to reflect its role as the general page renderer.
    // The previous implementation of `renderWandererPage` that filled `innerHTML` directly
    // is now part of `renderCurrentPage` and uses `pageTemplates`.
    // The `_getDOMElementsForPage` and `renderAllWandererComponents` calls are moved here.
};

// This part of the code (below this line) seems to be the WandererPageRenderer module definition.
// It was incorrectly nested inside WandererFeatures in the provided file.
// I've extracted it to be a separate export, as indicated by `export const WandererPageRenderer`.
// If these are indeed two separate modules, they should be defined separately at the top level.
// Based on the imports and usage, `WandererPageRenderer` is intended to be a distinct module.

export const WandererPageRenderer = { // This was the problematic section in previous check.
    pageTemplates: {},
    _tabClickHandler: null,
    _navClickHandler: null, // Added to store event listener reference for removal

    setDependencies(db, saveDB, uiM, worldM, gt) {
        dbInstanceRef = db; // Correct: Using dbInstanceRef
    ...
    // The rest of the functions from `renderWandererNav` down to `definePageTemplates`
    // are part of `WandererPageRenderer` and are already present in the provided code.
    // I will *not* duplicate them here, as they are part of the original file,
    // assuming the user's intention is to provide the full modified file.
    // The previous analysis and the fix involved changing `dbInstance` to `dbInstanceRef` within these functions.

    // ... (rest of the functions of WandererPageRenderer, e.g., renderWandererNav, setupWandererNavEvents, etc.)
    // ... (All functions like renderWandererNav, getArchetypePathAction, setupWandererNavEvents, renderCurrentPage,
    // _getDOMElementsForPage, renderAllWandererComponents, _applyCosmicTheme, renderPlayerStatus,
    // renderNpcList, addToWandererChronicle, renderChronicle, renderActiveQuestsLog, renderNotificationLog,
    // setupQuestLogTabs, renderInventoryPage, _showItemDetailPanel, _hideItemDetailPanel, _createActionButton,
    // _useItem, _equipItem, _discardItem, renderJournalPage, _addPersonalReflection, getWandererCharacterHtml,
    // renderWorldMap, _showRegionDetailPanel, _hideRegionDetailPanel, _showPoiDetailPanel, _hidePoiDetailPanel,
    // _handleMapZoom, renderSkillTree, and definePageTemplates
    // are assumed to be part of this `WandererPageRenderer` export based on the original file structure
    // and have been updated in the previous analysis to use `dbInstanceRef` where necessary.)
};

// Re-including the functions that were defined under `export const WandererPageRenderer = { ... }`
// in the original file, to make this response a complete, self-contained file.

// The code block for `WandererPageRenderer` starts from here and includes all methods within it.
// I'm embedding the rest of the functions from your original `wandererPageRenderer.js` file here,
// assuming they belong to `export const WandererPageRenderer`.
// The corrections for `dbInstance` to `dbInstanceRef` have been applied in my previous analysis.
// This is the full content of your `wandererPageRenderer.js` with the identified `dbInstance` references fixed.

// --- Start of WandererPageRenderer methods (as provided by user, with fixes) ---

WandererPageRenderer.renderWandererNav = function (currentPageId) {
    const navContainer = document.getElementById('wanderer-nav');
    if (!navContainer) return;

    const navItems = [
        { id: 'character', name: 'Karakter', icon: 'user' },
        { id: 'inventory', name: 'Inventaris', icon: 'package' },
        { id: 'journal', name: 'Jurnal', icon: 'book' }, // NEW: Journal link
        { id: 'quest_log', name: 'Kronik & Misi', icon: 'clipboard-list' }, // Renamed from Quest Log
        { id: 'world_map', name: 'Peta Dunia', icon: 'map' },
        { id: 'skill_tree', name: 'Pohon Skill', icon: 'git-branch' },
        { id: 'sanctuary', name: 'Sanctuary', icon: 'church' }
    ];
    // Tambahkan aksi jalur arketipe jika pengguna memiliki arketipe
    if (getCurrentUser() && getCurrentUser().archetype) {
        const archetypeAction = WandererPageRenderer.getArchetypePathAction(getCurrentUser().archetype);
        if (archetypeAction) {
            navItems.push({ id: archetypeAction.id, name: archetypeAction.name, icon: archetypeAction.icon });
        }
    }
    ...
    navContainer.querySelectorAll('.wanderer-nav-link').forEach(link => {
...
WandererPageRenderer.getArchetypePathAction = function (archetypeId) {
    const pathActions = {
        'inquisitor': { id: 'interrogate_action', name: 'Menginterogasi', icon: 'search' },
        'echo-scribe': { id: 'absorb_echo_action', name: 'Menyerap Gema', icon: 'eye' },
        'sentinel': { id: 'challenge_action', name: 'Menantang', icon: 'sword' },
...
WandererPageRenderer.setupWandererNavEvents = function () {
    const navLinks = document.querySelectorAll('.wanderer-nav-link');
    navLinks.forEach(link => {
        link.removeEventListener('click', WandererPageRenderer._navClickHandler); // Remove previous listener
            ...
            if (currentUser && currentUser.archetype) {
                ...
                switch (pageId) {
                    ...
                    case 'commission_action':
                        ...
                        return;
                    // Add other archetype actions here
                }
            }
...
// This is the `renderCurrentPage` function that replaced the old `renderWandererPage` logic.
WandererPageRenderer.renderCurrentPage = function (pageId) {
    // Hide all page containers first
    const allPageContainers = document.querySelectorAll('#wanderer-page-container > div'); // Assuming direct children are pages
    allPageContainers.forEach(container => {
        if (container.id.endsWith('-page') || container.id === 'quest-log-page') { // Ensure it's a page container
...
WandererPageRenderer._getDOMElementsForPage = function (pageId) {
    // Re-fetch common elements
    Object.assign(this, {
...
WandererPageRenderer.renderAllWandererComponents = function (pageId) {
    const currentUser = getCurrentUser();
...
WandererPageRenderer._applyCosmicTheme = function () {
    const body = document.body;
    const currentCycleEffects = WorldManagerRef.getCurrentCosmicCycleEffects();
    const theme = currentCycleEffects.worldVisualTheme || 'normal';
    ...
    const cosmicCycleNameDisplay = document.getElementById('cosmic-cycle-name');
    const cosmicCycleIconDisplay = document.getElementById('cosmic-cycle-icon');
    if (cosmicCycleNameDisplay && cosmicCycleIconDisplay && dbInstanceRef.world.cosmicCycle) { // Corrected: Using dbInstanceRef
        const currentCycleDef = COSMIC_CYCLES[dbInstanceRef.world.cosmicCycle.currentCycleId];
        cosmicCycleNameDisplay.textContent = currentCycleDef.name;
        cosmicCycleIconDisplay.setAttribute('data-feather', currentCycleDef.eventIcon || (theme === 'bright' ? 'sun' : (theme === 'dark' ? 'moon' : 'star')));
        if (typeof lucide !== 'undefined' && lucide.createIcons) {
...
WandererPageRenderer.renderPlayerStatus = function () {
    document.getElementById('wanderer-title').textContent = `Peringkat Jiwa ${getCurrentUser().soulRank}: ${getCurrentUser().title}`;
    const statusContainer = document.getElementById('player-status-container');
    statusContainer.innerHTML = `<span class="${getCurrentUser().status.color}">${getCurrentUser().status.text}</span>`;
    const wandererLegacyDisplay = document.getElementById('wanderer-legacy-display');
...
WandererPageRenderer.renderNpcList = function (regionId) {
    const npcListContainer = document.getElementById('npc-list-container');
    if (!npcListContainer) return;

    npcListContainer.innerHTML = '<h3 class="text-xl font-serif font-bold text-white mb-4">NPC di Wilayah Ini:</h3>';
    const npcsInRegion = Object.values(dbInstanceRef.npc_souls || {}).filter(npc => npc.currentRegion === regionId); // Corrected: Using dbInstanceRef
...
WandererPageRenderer.addToWandererChronicle = function (entry) {
    const currentUser = getCurrentUser();
    if (!currentUser.chronicle) {
        currentUser.chronicle = [];
    }
...
WandererPageRenderer.renderChronicle = function () {
    const container = document.getElementById('chronicle-container');
    if (!container) return;
...
WandererPageRenderer.renderActiveQuestsLog = function () {
    const container = document.getElementById('active-quests-log-container');
    if (!container) return;
...
WandererPageRenderer.renderNotificationLog = function () {
    const container = document.getElementById('notification-log-container');
    if (!container) return;
...
WandererPageRenderer.setupQuestLogTabs = function () {
    const tabsContainer = document.getElementById('quest-log-tabs');
    if (!tabsContainer) return;
...
WandererPageRenderer.renderInventoryPage = function () {
    if (!inventoryGrid) {
        console.error("Inventory grid element not found for rendering.");
        return;
    }
...
WandererPageRenderer._showItemDetailPanel = function (item) {
    if (!itemDetailPanel) return;
...
WandererPageRenderer._hideItemDetailPanel = function () {
    if (itemDetailPanel) {
        itemDetailPanel.style.display = 'none';
    }
};

WandererPageRenderer._createActionButton = function (container, text, onClickHandler) {
    const button = document.createElement('button');
    button.textContent = text;
    button.className = 'px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white text-sm rounded transition-colors shadow-md';
    button.addEventListener('click', onClickHandler);
...
WandererPageRenderer._useItem = async function (item) {
    UIManagerRef.showNotification(`Menggunakan ${item.name}. (Fungsionalitas belum penuh)`, "info");
};

WandererPageRenderer._equipItem = function (item) {
    UIManagerRef.showNotification(`Memasang ${item.name}. (Fungsionalitas belum penuh)`, "info");
};

WandererPageRenderer._discardItem = async function (item) {
    UIManagerRef.showNotification(`Membuang ${item.name}. (Fungsionalitas belum penuh)`, "info");
};

WandererPageRenderer.renderJournalPage = function () {
    if (!journalEntriesContainer) {
        console.error("Journal entries container element not found for rendering.");
        return;
    }
...
WandererPageRenderer._addPersonalReflection = async function () {
    if (!newReflectionText || !newReflectionText.value.trim()) {
        UIManagerRef.showNotification("Refleksi tidak boleh kosong!", 'alert-triangle', 'bg-red-500');
        return;
    }
...
WandererPageRenderer.getWandererCharacterHtml = function () {
    return `
        <div id="character-page-content" class="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div class="lg:col-span-2 space-y-8">
                <div class="glass-card p-8 rounded-2xl shadow-lg border border-border-color">
...
WandererPageRenderer.renderWorldMap = function () {
    const container = document.getElementById('world-map-page');
    if (!container) return;
    ...
    // Add pattern definitions to SVG (must be inside <defs>) - these are already in the container HTML template
    // No need to re-add defs here as they are part of the initial `container.innerHTML` assignment.
...
WandererPageRenderer._showRegionDetailPanel = function (regionId, mouseX, mouseY) {
    if (!regionDetailPanel) return;
    const region = dbInstanceRef.world.regions[regionId]; // Corrected: Using dbInstanceRef
...
WandererPageRenderer._hideRegionDetailPanel = function () {
    if (regionDetailPanel) {
        regionDetailPanel.classList.remove('visible');
        setTimeout(() => regionDetailPanel.style.display = 'none', 300);
    }
...
WandererPageRenderer._showPoiDetailPanel = function (poi, mouseX, mouseY) {
    if (!poiDetailPanel) return;
...
WandererPageRenderer._hidePoiDetailPanel = function () {
    if (poiDetailPanel) {
        poiDetailPanel.classList.remove('visible');
        setTimeout(() => poiDetailPanel.style.display = 'none', 300);
    }
...
WandererPageRenderer._handleMapZoom = function (delta) {
    currentZoomLevel = Math.max(0.5, Math.min(2.0, currentZoomLevel + delta));
    UIManagerRef.showNotification(`Zoom Peta: ${Math.round(currentZoomLevel * 100)}%`, 'zoom-in', 'info');
...
WandererPageRenderer.renderSkillTree = function () {
    const container = document.getElementById('skill-tree-page');
    if (!container) return;
...
WandererPageRenderer.definePageTemplates = function () {
    WandererPageRenderer.pageTemplates = {
        character: WandererPageRenderer.getWandererCharacterHtml(),
        inventory: `
            <div id="inventory-page" class="glass-card p-8 rounded-2xl shadow-lg border border-border-color h-full flex flex-col relative">
